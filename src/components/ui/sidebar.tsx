...
const SidebarProvider = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & {
  ...
  return <SidebarContext.Provider value={contextValue}>
        ...
        ...style
      } as React.CSSProperties} className={cn("group/sidebar-wrapper flex min-h-svh w-full has-[[data-variant=inset]]:bg-sidebar box-border", className)} ref={ref} {...props}>
            {children}
          </div>
        </TooltipProvider>
...
const Sidebar = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & {
  ...
  return <div ref={ref} className="group peer hidden md:block text-sidebar-foreground" data-state={state} data-collapsible={state === "collapsed" ? collapsible : ""} data-variant={variant} data-side={side}>
        ...
        <div className={cn("duration-200 fixed inset-y-0 z-10 h-svh w-[--sidebar-width] transition-[left,right,width] ease-linear md:flex", side === "left" ? "left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]" : "right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]", variant === "floating" || variant === "inset" ? "p-2 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]" : "group-data-[collapsible=icon]:w-[--sidebar-width-icon] group-data-[side=left]:border-r group-data-[side=right]:border-l", className)} {...props}>
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(({
  ...
  return <button ref={ref} data-sidebar="rail" aria-label="Toggle Sidebar" tabIndex={-1} onClick={toggleSidebar} title="Toggle Sidebar" className={cn("absolute inset-y-0 z-20 hidden w-0 transition-all ease-linear hover:bg-sidebar-border/10 group-data-[side=left]:-right-0 group-data-[side=right]:left-0 sm:flex", "[[data-side=left]_&]:cursor-w-resize [[data-side=right]_&]:cursor-e-resize", "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize", "group-data-[collapsible=offcanvas]:after:left-full", "[[data-side=left][data-collapsible=offcanvas]_&]:-right-0", "[[data-side=right][data-collapsible=offcanvas]_&]:-left-0", className)} {...props} />;
});
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<"main">>(({
  ...
  return <main ref={ref} className={cn("flex min-h-svh flex-1 flex-col bg-background", className)} {...props} />;
});
